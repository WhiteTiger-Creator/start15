...
# --------------------------------------------------------------------------
DECLARED_INPUTS = (
    DATA / "collector_journal.json",
    DATA / "event_snapshot_pre_truncation.json",
    DATA / "event_timeline.json",
    DATA / "sensor_registry.json",
    DATA / "triage_policy.json",
    SPEC_PATH,
    LOG_PATH,
    WORKFLOW_PATH,
...
def _hide_everything_else_under_app() -> tuple:
    ...
    The engine is compiled one file at a time, so a helper split into a sibling
    SOURCE never reaches the build -- but nothing stopped the engine reading or
    running that sibling at RUN time and letting it do the correlating. Whatever
    else is under /app goes into a root-owned stash for the duration of one
    graded run and comes back afterwards. A recovery program the agent kept is
    swept up with it, which is the point: the rebuild happens once, before the
    engine is run at all.
    """
