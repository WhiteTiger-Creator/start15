// Incident-correlation engine shipped before the response review.
package main
...
func main() {
	...
	sessionGap := pol.Default["session_gap_sec"]
	pivotMinHosts := int(pol.Default["pivot_min_hosts"])
	severityFloor := pol.Default["severity_floor"]
	...
	for _, group := range byHostAccount {
		...
		for i := 1; i < len(group); i++ {
			if group[i].ObservedTS-group[i-1].ObservedTS > sessionGap {
				sessionCount++
			}
		}
	}
